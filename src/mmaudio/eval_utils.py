...
from src.mmaudio.data.av_utils import VideoInfo, read_frames, reencode_with_audio
from src.mmaudio.model.flow_matching import FlowMatching
from src.mmaudio.model.networks import MMAudio
from src.mmaudio.model.sequence_config import (CONFIG_16K, CONFIG_44K, SequenceConfig)
from src.mmaudio.model.utils.features_utils import FeaturesUtils
from src.mmaudio.utils.download_utils import download_model_if_needed
...
class ModelConfig:
    ...
    mode: str
    synchformer_ckpt: Path = Path(MODEL_PREFIX + '/ext_weights/synchformer_state_dict.pth')
...
small_16k = ModelConfig(model_name='small_16k',
                        model_path=Path(MODEL_PREFIX + '/weights/mmaudio_small_16k.pth'),
                        vae_path=Path(MODEL_PREFIX + '/ext_weights/v1-16.pth'),
                        bigvgan_16k_path=Path(MODEL_PREFIX + '/ext_weights/best_netG.pt'),
                        mode='16k')
small_44k = ModelConfig(model_name='small_44k',
                        model_path=Path(MODEL_PREFIX + '/weights/mmaudio_small_44k.pth'),
                        vae_path=Path(MODEL_PREFIX + '/ext_weights/v1-44.pth'),
                        bigvgan_16k_path=None,
                        mode='44k')
medium_44k = ModelConfig(model_name='medium_44k',
                         model_path=Path(MODEL_PREFIX + '/weights/mmaudio_medium_44k.pth'),
                         vae_path=Path(MODEL_PREFIX + '/ext_weights/v1-44.pth'),
                         bigvgan_16k_path=None,
                         mode='44k')
large_44k = ModelConfig(model_name='large_44k',
                        model_path=Path(MODEL_PREFIX + '/weights/mmaudio_large_44k.pth'),
                        vae_path=Path(MODEL_PREFIX + '/ext_weights/v1-44.pth'),
                        bigvgan_16k_path=None,
                        mode='44k')
large_44k_v2 = ModelConfig(model_name='large_44k_v2',
                           model_path=Path(MODEL_PREFIX + '/weights/mmaudio_large_44k_v2.pth'),
                           vae_path=Path(MODEL_PREFIX + '/ext_weights/v1-44.pth'),
                           bigvgan_16k_path=None,
                           mode='44k')
